...
void CLI_task(void * params){
	...
	while(1){
		...
		HAL_UART_Transmit_DMA(base.huart, base.transmitBuff, strnlen((char*) base.transmitBuff, CLI_MAX_LINE_LEN));
	}
}
    ...
    if(huart == base.huart){
    	...
    	if(base.receivedBuff[base.receivedIndex] == BACKSPACE){
        	if(base.receivedIndex > 0){
        		...
        		xQueueSendToBackFromISR(base.txQueue, "\b \b", &higherPriorityTaskWoken);
        		base.receivedIndex--;
        	}
			...
			/* Print received character and store it in the buffer. */
			 ...
			 HAL_UART_Transmit_IT(base.huart, (uint8_t *)&base.receivedBuff[base.receivedIndex], 1);
		...
		HAL_UART_Receive_IT(base.huart, (uint8_t *)&base.receivedBuff[base.receivedIndex], 1);
