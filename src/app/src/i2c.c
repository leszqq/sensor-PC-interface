...
void I2C_init()
{
    ...
    GPIOA->MODER |= (0x2UL << GPIO_MODER_MODER9_Pos) | (0x2UL << GPIO_MODER_MODER10_Pos);				// alternate function mode
    GPIOA->OTYPER |= GPIO_OTYPER_OT_9 | GPIO_OTYPER_OT_10;												// open drain
    GPIOA->PUPDR |= (0x1UL << GPIO_PUPDR_PUPDR9_Pos) | (0x1UL << GPIO_PUPDR_PUPDR10_Pos);				// pull ups
    GPIOA->AFR[1] |= (GPIO_AF4_I2C2 << GPIO_AFRH_AFRH1_Pos) | (GPIO_AF4_I2C2 << GPIO_AFRH_AFRH2_Pos);
